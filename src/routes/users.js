...
router.route('/')
    .get((req, res, next) => {
      userController.list().then(list => res.send(
          list.map(user => cleanup(user)))
      ).catch(next);
    })
    .post((req, res, next) => {
      userController.create(req.body).then(created => {
        res.status(201).send(cleanup(created));
      }).catch(next);
    });

router.route('/:id')
    .get((req, res, next) => {
      res.send(cleanup(req.user));
    })
    .put((req, res, next) => {
      userController.update(req.user._id, req.body).then(updated => {
        res.status(200).send(cleanup(updated));
      }).catch(next);
    })
    .delete((req, res, next) => {
      userController.delete(req.user._id).then((deleted) => {
        res.status(200).send(cleanup(deleted));
      }).catch(next);
    });
